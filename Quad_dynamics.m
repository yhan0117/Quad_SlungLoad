...
syms I_1 I_2 I_3
syms m_g m_p d l g C_d C_l
param = [I_1 I_2 I_3 m_g m_p d l g C_d C_l];
I_g = [I_1   0   0;
       0    I_2  0;
       0    0   I_3];
...
% Pendulum energy
T_pend = m_p*sum(v_po.*v_po)/2;
U_pend = m_p*g*sum(r_po.*[0 0 1].');
...
RHS = t*zeros(1,8);
for i = 1:8
    RHS(i) = simplify(Q(q(i), r, u, C_l, R_IB, R_IC, T, r_go, r_po), "Steps", 5);
end
...
%--------------------------------------Functions--------------------------------------%
% Generalized Forces
function Gen_F = Q(var, r, u, Cl, R_IB, R_IC, T, r_go, r_po)
    Gen_F = 0;
    for i = 1:4
